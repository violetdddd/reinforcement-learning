...
class five_tigers():
    ...
    def __init__(self, initial=[[EMPTY, EMPTY, EMPTY, EMPTY, EMPTY],
        ...
        Each game board has
            - `board`: 5*5 board containing X or O or EMPTY
            - `player`: 0 or 1 to indicate which player's turn
            - `winner`: None, 0, or 1 to indicate who the winner is
            - `left`: EMPTY grid number in game board
            - `scores`: record scores for both player.scores[0] for first player
        """
    ...
    def now_player_sign(self):
        """
        return the current player's sign in game board
        """
        if self.player == 0:
            return X
        else:
    ...
    def update_score(self, action):
        """
        update the score after each player take an action
        """
        # the related possible point to get for each move
        related = {
            (0, 0): [1, 6, 11, 21, 30, 31, ],
            (0, 1): [1, 7, 13, 22, 31, 32, ],
        ...
        # if some pattern is made, add the score for current player
        ...
        # get the action and current player's sign
        i, j = action
    ...
    def check_winner(self):
        """
        if the game is over, return the winner. 2 means tie
        """
...
def train(n):
    ...
    # list can't be the key for dict, we can use tuple containing numbers. 1 for self taken; 0 for rival taken 
    ...
    for i in range(n):
        ...
        # adjust epsilon through training to balance exploration and exploitation
        if 300000 < i < 500000:
            player.epsilon = 0.5
        ...
        # Keep track of last move made by either player and the consequence scores increase
        last = {
        ...
        while True:
            ...
            # 10 for winner; -20 for loser to avoid lose more than try to win
            if game.winner is not None:  # actually now game.player must be 1 because the first player makes last move
                new_state_last = board_to_state(1-game.player, copy_new_state)
                if game.winner == 1:  # second player win
                ...
                elif game.winner == 0:  # first player win
                ...
                else:  # tie. Maybe a small -1 can push ai to try to be aggressive and to win
            ...
            # If game is continuing, get reward for scores players get more than the rival
