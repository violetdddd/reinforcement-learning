...
class Tictactoe():
    ...
    def __init__(self, initial=[[EMPTY, EMPTY, EMPTY],
        ...
        Each game board has
            - `board`: 3*3 board containing X or O or EMPTY
            - `player`: 0 or 1 to indicate which player's turn. first player is 0
    ...
    def available_actions(cls, board):
        """
        Tictactoe.available_actions(board) takes a `board` 2dlist as input
    ...
    def check_winner(self):
        """
        check if the game is over. If so, return the winner, otherwise return none
        """
        # a full row
        for i in range(3):
    ...
    for i in range(n):
        ...
        while True:
            ...
            if game.winner is not None:
                ...
                # winner must be the last player to take a move, so this actually can't happen
                ...
                # 3 for winner; -10 for loser to avoid lose more than try to win
            ...
            # If game is tie, no rewards yet. It may be useful if we give both player little reward like 0.1
            elif game.left == 0:
                new_state_last = board_to_state(1-game.player, copy_new_state)
                player.update(state, action, new_state_last, 0)
                player.update(
                    last[game.player]["state"],
                    last[game.player]["action"],
                    new_state,
